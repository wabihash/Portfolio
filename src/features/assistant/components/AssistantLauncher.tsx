...
export function AssistantLauncher({
  ...
  return (
    <div className={`relative flex flex-col items-end ${showGreeting ? 'gap-0.5 sm:gap-1' : 'gap-2.5 sm:gap-3'}`}>
      {showGreeting ? (
        <section className="relative w-[min(14rem,calc(100vw-1rem))] rounded-xl border border-cyan-200/20 bg-[#07142b]/95 p-2.5 text-white shadow-[0_18px_42px_rgba(0,0,0,0.55)] backdrop-blur-xl transition-all duration-300 ease-out animate-in fade-in zoom-in-95 slide-in-from-bottom-2 after:content-[''] after:absolute after:-bottom-1 after:right-7 after:h-3 after:w-3 after:rotate-45 after:border-b after:border-r after:border-cyan-200/20 after:bg-[#07142b] sm:w-[min(16rem,calc(100vw-2rem))] sm:rounded-2xl sm:p-3 sm:after:right-8">
          <button
            type="button"
            onClick={onDismissGreeting}
            className="absolute right-2 top-2 rounded-md border border-white/15 px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-white/80 hover:bg-white/10"
            aria-label="Dismiss assistant greeting"
          >
            x
          </button>
          <p className="pr-7 text-[11px] font-semibold text-cyan-100 sm:text-sm">
            {greetingText}
            <motion.span
              aria-hidden="true"
