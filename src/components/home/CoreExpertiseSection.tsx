      ...
      <div className="section-card p-4 sm:p-5 md:p-8">
        ...
        <div className="mt-5 flex flex-wrap gap-2.5 sm:gap-3">
          {CORE_EXPERTISE_BADGES.map((badge) => (
            <span
              key={badge}
              className="rounded-full border px-3 py-1.5 text-xs transition hover:-translate-y-0.5 hover:border-[#fbbf24]/40 hover:shadow-[0_8px_30px_rgba(251,191,36,0.2)] sm:px-4 sm:py-2 sm:text-sm"
        ...
        <div className="mt-7 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 sm:gap-4">
          {CORE_EXPERTISE_STATS.map((item) => (
            <article key={item.label} className="inset-card p-4 transition-transform duration-300 hover:-translate-y-0.5 sm:p-5">
